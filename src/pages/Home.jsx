...
import { getMachines, setMachines } from "../components/Dataset";
import savi from "@icons/savi.jpg";

const Home = ({ color, light }) => {
  let { searchValue, setSearchValue, searchedText } = SearchHook();
  ...
  const getMachinesFromGoogle = async () => {
    let data = await getMachines();
    setMachines(data);
    setisLoading(false);
  };

  useEffect(() => {
    getMachinesFromGoogle();
  }, []);
